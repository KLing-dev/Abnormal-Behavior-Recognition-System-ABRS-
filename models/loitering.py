...
class AreaLoitering(Base):
    __tablename__ = "t_area_loitering"
    ...
    threshold_min = Column(Integer, default=10, comment="徘徊时长阈值")
    is_enable = Column(Integer, default=1, comment="1启用/0禁用")
    create_time = Column(DateTime, default=datetime.now, comment="创建时间")


class AlarmLoitering(Base):
    __tablename__ = "t_alarm_loitering"

    id = Column(Integer, primary_key=True, autoincrement=True)
    alarm_time = Column(DateTime, nullable=False, comment="告警时间")
    event_id = Column(String(2), default="03", comment="事件编号")
